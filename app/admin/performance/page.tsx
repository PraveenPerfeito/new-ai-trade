...
function TrackRecordTab({ data, loading }: { data: import('@/lib/admin-api').TrackRecordResponse | null; loading: boolean }) {
  ...
  const expCls = (ex: number | null) => ex == null ? 'text-terminal-muted' : ex >= 0.5 ? 'text-bull-default' : ex >= 0.2 ? 'text-blue-400' : ex >= 0 ? 'text-amber-400' : 'text-bear-default'
...
export default function PerformancePage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-terminal-text text-xl font-semibold">Performance</h1>
        <p className="text-terminal-muted text-sm mt-1">Track record · Edge validation · Attribution intelligence</p>
      </div>
