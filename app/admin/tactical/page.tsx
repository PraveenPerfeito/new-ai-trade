  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-700 transition-colors">
      ...
      >
        ...
        {/* Context strip — fills ALL gap on lg+, always has content */}
        <div className="hidden lg:flex items-center gap-2 flex-1 min-w-0 overflow-hidden mx-2">
          {/* Setup description — always present */}
          {sig.setupDescription && (
            <p className="text-[10px] text-zinc-700 font-mono truncate flex-1 min-w-0 leading-none">
              {sig.setupDescription.split('|')[0].trim()}
            </p>
          )}
          {(sig as any).breakoutStrength && (
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-emerald-500/15 bg-emerald-500/5 text-emerald-500/50 whitespace-nowrap shrink-0">
          ...
          {(sig as any).oiInterpretation && (sig as any).oiInterpretation !== 'NEUTRAL' && (
            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded border border-zinc-700/40 text-zinc-600 whitespace-nowrap shrink-0">
