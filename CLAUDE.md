...
37. **REDIS.OPTIMIZE.1 — Cost reduction audit (June 2026)** — Baseline ~430K ops/month, target <250K. 7 fixes identified; P0 (O3/O4/O5/O6) alone saves ~100K ops/month. Top hotspots: Railway health checks (PING+GET every ~60s = ~86K ops/month), quota snapshot write on every monitoring call (~42K), heartbeat every 60s (~43K), settings gen check every 60s (~43K), dead `monitor:scan_durations` list (~14K). Status: PENDING implementation. See `docs/REDIS_OPTIMIZATION_AUDIT.md` § Phase REDIS.OPTIMIZE.1.
38. **OPS.CONSOLIDATION.1 — Full ops optimization (June 2026)** — Supersedes REDIS.OPTIMIZE.1. Target: <200K Redis ops/month + <39K CloudAMQP msgs/month + 13 → 3 admin pages. 6 implementation batches: P0 one-liners (R1–R5, ~116K ops saved), health cache + polling (R6–R7, ~103K), CloudAMQP beats (A1–A3, ~23K msgs saved), hit/miss removal (R8–R9, ~47K), dashboard 3-page consolidation, provider table + Signal Quality Scorecard. GO decision confirmed. See `docs/OPS_CONSOLIDATION_1.md`.
