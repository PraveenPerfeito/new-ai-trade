...
26. **Gate rejection tracking** — `gate_rejections: dict[str, int]` collected per scan in `orchestrator.py`, persisted to `scan_metrics_log` via `record_scan()`. 23 canonical gate keys in `GATE_REJECTION_KEYS`: `BTC_DOWN_BUY`, `TOXIC_DENYLIST`, `SIGNAL_COOLDOWN` (was `DUPLICATE_SIGNAL` — renamed in SIGNAL.COOLDOWN.1), `CONFIDENCE_REJECTION`, `CMC_REJECTION`, `REGIME_REJECTION`, `CONTRA_REGIME_REJECTION`, `SIDEWAYS_REJECTION`, `KLINE_EMPTY`, `KLINE_PARTIAL`, `MTF_REJECTION`, `VOLATILITY_REJECTION`, `TREND_STRENGTH_REJECTION`, `SETUP_REJECTION`, `RR_REJECTION`, `RISK_REJECTION`, plus 7 `ms_*` market-structure sub-keys. Visible in System page `GateRejectionGrid`. `normalize_gate_rejections()` in `scan_metrics.py` maps aliases (e.g. `btc_context` → `BTC_DOWN_BUY`, `duplicate_signal` → `SIGNAL_COOLDOWN`).
...
28. **Celery broker — CloudAMQP (AMQP), not Redis** — `CELERY_BROKER_URL=amqps://...@cloudamqp.com` set in Railway. Eliminates ~34,560 Redis BLPOP ops/day (the dominant consumer). `celery_app.py` checks `broker_use_ssl` and `redis_backend_use_ssl` independently so switching broker to AMQP doesn't break Redis Cloud result backend SSL. `SchedulerCoordinator` uses `redis_url` directly (not `broker_url`) — distributed locks, enable/disable state, and scan timestamps are unaffected by broker change.
29. **Celery result backend — `rpc://`** — `CELERY_RESULT_BACKEND=rpc://` set in Railway. Stores task results in CloudAMQP instead of Redis. Zero Redis ops for result storage. Safe: no code calls `AsyncResult.get()` on scheduled task results.
...
47. **SIGNAL.QUALITY.3 + dashboard truth fixes (June 2026)** — Pipeline: (1) **RSI divergence detection** in `detect_setup()` — compares last 6 vs prior 6 candle swing highs/lows with RSI at both points (`calc_rsi(candles_1h[:-6])`); divergence AGAINST signal→−10, divergence IN FAVOUR (classic reversal entry)→+8; requires ≥40 1h candles. (2) **Counter-EMA200 penalty** — BUY below / SELL above 1h EMA200 (direction_reliable) →−8; was previously free. (3) **CMC direct Python fallback** — `_fallback_cmc_direct()` in `intelligence_cache.py`: when Redis intel cache cold, Python calls CMC listings API directly (uses `COINMARKETCAP_API_KEY`) BEFORE CoinGecko; also last-resort after CoinGecko failure. Root cause: scans completing in ~6s with 0 coins when cache cold + CoinGecko failing. (4) **TELEGRAM_SENT lifecycle stage** — `computeLifecycleStage()` returns TELEGRAM_SENT for first 30 min after send, then ACTIVE, then STALE. Dashboard truth: LifecycleFunnel `sent` now uses real `telegramSent` boolean (outcome-stage inference overcounted — outcomes register for ALL signals); funnel "Approved" step removed (always ~100% — every persisted signal is validated; **VALIDATED stage is unreachable by design**) replaced with AI/Screened chips; ANALYZED removed from won counts (never produced); win-rate shows sample size; `TradeStructureBar` proportional RR-true scaling (was SL×3/TP×2 distortion) + negative-distance guards; resolved Tactical rows show realized `rrAchieved`/`pnlPct`/`durationHours` (tactical route now selects them from signal_outcomes; fields added to `TacticalSignalRow`).
51. **SETTINGS.SIMPLIFY.1 (June 2026)** — Founder Control Center reduced from 5 sections + 3 accordions to 3 sections + 1 accordion. Page order: FounderSummaryCard → **Quick Controls** (audited daily set, always visible: `ai.enabled` + `telegram.alerts_enabled` + `features.apply_founder_thresholds` toggles, then `scanner.min_confidence`/`scanner.alert_confidence`/`signals.min_rr_ratio`/`scanner.max_coins_per_run` via TacticalControlCard) → Operating Mode (3 primary cards + 4 specialist preset **chips**, no accordion; sub-85-confidence presets carry an ALPHA.TRUTH.1 warning) → "All Settings & Audit Log" single collapsed accordion. Removed: ActiveSettingsSummary, Advanced Presets + Advanced Settings accordions.
60. **PLATFORM.SIMPLIFICATION.1 (June 2026, UI-only)** — Complexity audit of all 5 admin pages (25 tabs, ~100 sections, 33 API endpoints, 7,274 lines). Classified every widget/card/section as KEEP/MERGE/HIDE/REMOVE. Key removals: News Tab (XAI_API_KEY unset → already 503 in prod), MarketStructureBreakdown (MARKET_STRUCTURE.FIX.1 validation complete — dead telemetry), StageLegend, Pipeline Integrity Card, InfraConfigSection, static reference cards, duplicate DimTables (Signal State/Extension Risk), Intelligence Validation Section (POSTFIX.1 staging). Key merges: Tactical Tab → Signals Tab; Probability Tab → Track Record Tab; duplicate ProviderHealthTable/GateRejectionGrid → System only; BTC Regime card → Trading Overview only. Key hides: 20 items behind collapsed accordions (Grade Validation Strip, Cache Tab, Sectors Tab, Market Tab, AlphaWatchlist, Feature Flags from Settings, etc.). Result targets: 25 tabs → 14 (−44%), signal card 32 fields → 13 primary (−59%), polling endpoints ~33 → ~20 (−39%). Zero backend changes. See `docs/PLATFORM_SIMPLIFICATION_1.md`.
59. **VALUE.SURFACING.3 (June 2026, commit `cc3a63f`, UI-only)** — Closed final gaps from `docs/VALUE_MAXIMIZATION_1.md` audit. (1) **Liquidation zones** — `futuresData.liquidationZones[]` now rendered in `IntelligencePanel` as directional chips: `↓ $94,200 · 2.1% away (strong)` in red for `LONG_LIQ`, emerald for `SHORT_LIQ`; up to 4 zones shown per futures signal. (2) **Per-Coin Performance table** — `coin_performance` was already included in the full `/analytics/edge/report` payload (Python `generate_edge_validation_report()` line 714) but the TypeScript `EdgeReport` interface didn't declare it; added `coin_performance` field + `CoinStats` interface to `lib/admin-api.ts`; Analytics→Edge tab now shows top-10 coins by expectancy with resolved/WR/Exp/PF/MaxDD columns and `top WR` / `high DD` badges. Zero new API calls. Also confirmed: `edge/calibration` is NOT orphaned — already consumed by the Calibration tab via `EdgeReport.confidence_calibration`. **All 7 listed DB migrations confirmed applied 2026-06-16** (verified in MASTER_PLATFORM_STATUS.md): `probability-gate-migration.sql`, `probability-engine-migration.sql`, `telegram-delivery-migration.sql`, `validation-source-migration.sql`, `ai-call-log-trace-migration.sql`, `attribution-snapshots-migration.sql`, `signal-outcomes-regime-migration.sql` — all idempotent (IF NOT EXISTS). For fresh deploy, run all files in `database/` via Supabase SQL Editor.
58. **VALUE.SURFACING.1+2 — presentation sprint (June 2026, commits `969d430` + `d637674`)** — Surfaced 23 fields that were computed, stored in DB, fetched by API, and silently discarded by the UI. **Zero signal generation changes. Zero new AI calls. Zero new DB tables.** Sprints completed in two passes against `docs/VALUE_MAXIMIZATION_1.md` audit. **SURFACING.1** (Phases A–J): `FounderCommandCenter` in Trading Overview wires the orphaned `/analytics/track-record` endpoint (7d/30d/90d WR/Exp/PF + by-mode + probability accuracy); `IntelligencePanel` expanded with empirical WR/N/grade chip, continuation probability + entry quality + institutional score + regime adj, full-length AI reasoning; `GradeValidationStrip` shows empirical vs heuristic grade WR; `FreshnessTag` on ACTIVE signals (⏱ N h left); RSI + volume spike in technical section; funding rate + OI trend + L/S ratio + max leverage in futures section; `AlphaWatchlist` component + `/api/signals/watchlist` route; Telegram grade line extended with `Hist: WR% (n=N)` + `Cont: X%`. **SURFACING.2**: EMA200 ABOVE/BELOW (Python-side field — cast via `sig.indicators as unknown as Record<string,unknown>`), candle_pattern, `⚡ BB SQUEEZE` chip; futures section extended with `fundingBias`, `fundingRateAnnualized`, `oiChange24h`, `momentumScore`; `mcapTier`, `extensionRisk`, `pullbackQuality` added to intel row; `aiExplainability.summary` (one-line trade thesis), `.continuationCase` (↗ green quote), `.cautionCase` (⚠ amber quote); `risks[]` red chips + `strengths[]` green chips; `EdgeReport` type extended with `scanner_mode_analysis` + `market_regime_analysis` (both already in `/analytics/edge/report` payload but untyped/unrendered); Scanner Mode Performance table + Regime Performance table added to Analytics→Edge tab. See `docs/VALUE_SURFACING_1.md`.
57. **PERFORMANCE.VERIFICATION.1 (June 2026, analytics-only)** — Read-only validation of the Probability Engine / RiskGrade 2.0 / Edge Matrix. `backend/analytics/performance_verification.py` + `GET /api/analytics/performance-verification` + Verification section in Analytics→Probability: probability accuracy (predicted vs realized + drift + Wilson-CI calibration tick per regime/grade/breakout/type/mode), grade validation with **inversion detection** (adjacent populated grades, n≥10), edge stability (top `regime|type|breakout` cohorts across 7/30/90d, Jaccard + top-3 retention), sample quality with promotion targets. Live results: **empirical grades perfectly monotonic, zero inversions** (A+ 73.5%/+1.286R/PF 5.85 → D 13.6%/−0.581R/PF 0.33, n=1,822 in-sample) while heuristic A–F fails the same test (A 33.9% < B 36.1% < C 56.4%); probability accuracy n=1 resolved stamped (far below the 200 target). **Promotion criteria codified**: `probability_gate_v1` needs ≥200 resolved stamped + MAE ≤0.25 + drift ±10pp + all n≥30 cells calibrated; `riskgrade_v2` needs ≥30 stamped/grade in ≥3 buckets + zero stamped inversions + A+/A ≥ +0.3R vs baseline. Both flags remain OFF. 7v30 cohort divergence is regime-driven by design (cohorts are regime-conditional); 30v90 overlap trivial until outcome history exceeds 30d. 9 tests in `test_performance_verification.py`. See `docs/PERFORMANCE_VERIFICATION_1.md`.
56. **PHASE.9.P1.PROBABILITY.ENGINE.1 (June 2026)** — Outcome-derived probability becomes primary (confidence stays visible). `probability.py`: `evaluate()` → `CohortStats{wr,exp,pf,n,level,ci}` over a **5-level hierarchy** (`regime|type|breakout`→`regime|type`→`regime`→`conf_band`→`global`, n≥30/level) with **Wilson 95% CI**; `empirical_grade()` bins cohort expectancy into A+/A/B+/B/C/D (≥1.0/0.6/0.35/0.15/0, n≥30); `should_suppress_send()` extended with AND-expectancy filter. `outcome_learning.py`: +4 dims (`global`, `trend_tier|breakout`, `sector|funding`, `oi|positioning`) = 23 sets. Signals stamped `empirical_grade` (shadow; migration `database/probability-engine-migration.sql`, tolerant writes). Flags: `probability_gate_v1` (expectancy filter, **OFF**) + `riskgrade_v2` (display-primary empirical grades, **OFF**) + `scanner.min_empirical_exp` (0.0). APIs: `GET /api/analytics/edge-matrix` (top-50 combos by exp, Wilson CI, 12 pair/triple dims) + `GET /api/analytics/track-record` (7/30/90d WR/PF/exp + per-mode + probability accuracy = predicted-vs-realized). UI: Analytics→**Probability** tab; `P {wr}%` chips in Signals/Tactical rows (`rowToSignal` maps empirical_wr/n/grade). 30d simulation: WR≥45 gate keeps 43.5% volume at WR 59.6% (+23.5pp), exp +0.877R, PF 3.17. 21 tests in `backend/analytics/tests/test_probability_engine.py`. See `docs/PHASE_9_P1_PROBABILITY_ENGINE_1.md`.
55. **TELEGRAM.RELIABILITY.1 (June 2026)** — P0 delivery fixes from TELEGRAM.DELIVERY.AUDIT.1 (audited: ~25% of queued alerts tail-lost; 76/83 eligible-unsent were dedup shadows). (a) **WS1 queue drain** — `flush_queue(timeout_s)` in `telegram_notifier.py` awaits `queue.join()` before event-loop exit; hooked in `scan_task._run_and_record` finally-block (30s) + `analytics_tasks._with_telegram_flush` wrapper (15s); queue+worker are now **per-event-loop** (recreated when loop changes, like the asyncpg pool). (b) **WS2 delivery ground truth** — `_QueueItem(text, signal_id, dedup_key)`; drain worker writes `signals.telegram_delivered` + `telegram_delivery_error` post-send (best-effort, tolerates missing migration `database/telegram-delivery-migration.sql`). Semantics: `telegram_sent`=queued, `telegram_delivered` true/false/NULL(pre-fix). (c) **WS3 dedup-after-delivery** — `_is_duplicate_alert()` is check-only; cooldown SETEX moved to `_mark_alert_cooldown()` called ONLY on confirmed 200 — failed/lost sends no longer poison the 1h cooldown. (d) **WS4** — `_get_semaphore()`/`_get_rate_limiter()` in `ai_validator.py` recreate per running loop (fixes audited 'Semaphore bound to a different event loop' Claude crashes). (e) **WS5** — `GET /api/analytics/telegram-delivery` (24h/7d: generated/eligible/queued/delivered/failed/unresolved/shadowed/other) + `TelegramDeliveryCard` in Trading→Scanner. NO scanner/alpha logic touched. **P1 follow-ups (same phase):** (f) `_parse_claude_json()` truncation-aware JSON repair (fence strip, trailing commas, brace/string balancing) + `ai.max_tokens` setting WIRED with a 768 floor (was hardcoded 768 while the DB default was 500 — avg completion ~625 tokens made truncation the audited json_parse_failed cause). (g) `ai_call_log` +`symbol`+`setup_score` (tolerant insert; migration `database/ai-call-log-trace-migration.sql`) — Claude kill-gate value becomes measurable. (h) **Quality-aware dedup**: cooldown key stores delivered confidence; a duplicate with confidence ≥ previous + `DEDUP_UPGRADE_DELTA` (5) is delivered as an ⬆ UPGRADE alert; legacy/garbage cooldown values block unconditionally. (i) Tactical feed unified to one `useSharedPolling` key (`trading:tactical-feed`, limit=100 @120s) across SignalsTab/TacticalTab/Overview; System anomaly+burn-in polls 60→120s; `monitor:scan_durations` orphan Redis key deleted. 17 tests in `test_telegram_reliability.py`.
54. **P1.INTELLIGENCE.FIXES (June 2026)** — Four audit-deferred items fixed. (1) **FUNDING.TREND.FIX.1** — `_classify_funding_trend()` threshold was unreachable (0.0002 absolute vs ~0.0001 typical funding levels and ~1h reading windows → 100% STABLE over 30d); now `max(3e-5 absolute, 25% of |oldest|)` — restores the designed RISING×1.3/FALLING×0.7 adverse-funding multipliers. (2) **INTEL.PROPAGATE.1** — `build_intelligence_maps()` in `trending_universe.py` (pure core `_intelligence_maps_core`) builds sector_status + trend_score maps for ALL modes from cache only (RS via CMC 1h/24h proxies, no API calls); orchestrator step 2b runs it for non-TRENDING modes (sector_status was 100% NULL, trend_score 98.7% NULL in outcomes). (3) **high_confidence retirement flag** — `features.high_confidence_mode_enabled` default **ON** (no change); OFF skips that mode's scans via `_check_operational_flags(mode)` (audit: 26.8% WR 30d, 0/9 last week). (4) **PHASE.9.1 probability gate** — `backend/analytics/probability.py`: hourly-cached lookup over latest `attribution_snapshots` generation (`regime|type|breakout`→`regime|type`→`regime`, n≥30), `Signal.empirical_wr/empirical_n` stamped on every accepted signal (shadow data; best-effort persist tolerates missing migration `database/probability-gate-migration.sql`), `should_suppress_send()` withholds ONLY Telegram delivery when flag `probability_gate_enabled` (default **OFF**) is on and cohort WR < `scanner.min_empirical_wr` (default 45.0); unknown probability NEVER gates; suppressions logged + counted (`gate="probability_send_gate"`). 15 tests in `test_p1_intelligence_fixes.py`.
53. **SETTINGS.CENTER.2 — Founder Settings (June 2026, UI-only)** — Settings page reorganized by usage tier via `lib/settings-tiers.ts` (`settingTier(group,key)` → daily/occasional/engineering/dead — single source shared by Settings + System pages). Page: SafetyStatusCard (green when normal; red list of dangerous conditions from `DANGEROUS_FLAGS` — emergency stop ON, maintenance ON, telegram/AI OFF, alerts OFF, min_confidence<85 with floors ON) → Active Configuration summary (+ last-changed by/when from group metas) → Quick Controls (3 daily toggles) → Signal Quality (4 floor numbers) → Operating Mode → **Feature Flags grid on-page** (features group, minus quick toggles) → "Advanced Settings & Audit Log" accordion (occasional-tier fields only; `infra` + `features` tabs removed; dead fields `signals.confidence_high/medium` hidden). **Engineering tier** (infra group + set-once fields like ai.max_tokens/temperature/timeouts, telegram rate caps, scanner.delay_ms) relocated to System → `InfraConfigSection` accordion, READ-ONLY table. Backend untouched: settings service, autosave, audit log, pub/sub, reset-to-defaults all preserved. quota/providers/failover/market_cache were audited: they are NOT settings groups — env/code-controlled (CMC key, provider priority in `manager.ts`, cache TTLs in `lib/cache.ts`), surfaced read-only in Intelligence center.
52. **SETTINGS.WIRE.1 — settings wiring truth + founder floors (June 2026)** — Cross-check found most settings groups were PLACEBO: only `features`, `ai`, `telegram`, `anomaly` (via `propagation.apply_group_to_modules` → `anomaly_detector.configure()` hot-swap) and `scanner.trending_watchlist` are read by the backend; ALL scanner numerics + entire `signals`/`risk`/`infra` groups had zero consumers (scanner thresholds live in hardcoded `CONFIGS` + env `scanner_min_confidence_alert`). Fixes: (1) **`apply_founder_floors()`** in `orchestrator.py` + flag `features.apply_founder_thresholds` (default **OFF**) — applies Quick Controls as FLOORS on per-mode CONFIGS (`max()` for min_confidence/min_rr/alert threshold, `min()` for max_coins); floors can tighten but NEVER loosen below ALPHA.TRUTH.1 per-mode minimums (Aggressive preset's 72 cannot undo spot's 85); group defaults are a provable no-op (test-locked per mode). (2) **Wiring truth chips** in the All Settings table: `live` / `floor` / `display only` per field (`wiredState()` map in settings page). (3) **Recommended-value chips** (`RECOMMENDED` map: min_confidence 85, alert 85, coins 80, RR 2.0, ai.enabled, alerts_enabled) with one-click apply when current ≠ recommended. `anomaly` group is NOT hidden (initial SIMPLIFY.1 hide was wrong — propagation wires it). 5 tests in `test_founder_floors.py`.
50. **CONFIDENCE.CALIBRATION.2 (June 2026, read-only)** — Empirical confidence measurement; flag `confidence_calibration_v2` default **OFF**. `backend/analytics/confidence_calibration.py`: spec bands (`<80/80-84/85-89/90-94/95-100`), per-band n/WR/exp/PF/avg-winner/avg-loser/mean-stated, **drift = actual WR − mean stated confidence**, drift by regime/type/mode, founder insights (most over/underrated, best/worst actual; n≥30 only), hierarchical empirical lookup (`band|regime|type` → `band|regime` → `band` → global, n≥30/level — the Probability Engine foundation), data-quality block (NULL-regime %, single-regime warning, snapshot coverage). API: `GET /api/analytics/confidence-calibration` returns `{enabled:false}` when flag OFF. UI: `ConfidenceCalibrationSection` in Analytics→Calibration renders nothing when OFF. `outcome_learning.py` conf bands unified to spec banding + `mode|conf_band`/`type|conf_band` dimension sets added (19 total). ZERO production-path changes — confidence/scoring/gating untouched. Audit (30d): stated 95-100 → 35.5% WR global/44.2% clean; 85-89 → 42.1%/57.6%; 90-94 worst (31.4%). 13 tests in `backend/analytics/tests/test_confidence_calibration.py`. See `docs/CONFIDENCE_CALIBRATION_2.md`.
49. **PHASE.9.P0.EXPECTANCY.RECOVERY.1 (June 2026)** — P0 hardening from the PHASE.9 audit. (a) **OUTPUT.COLLAPSE.ALERT.1** — `check_output_collapse()` in `monitoring.py` runs after every scan cycle (hooked in `scan_task._run_and_record`); breach = signals_24h < 25% of 7d daily avg (baseline floor 3/day); 2 consecutive breaches → Telegram + log + System-page banner (`MonitorSnapshot.output_collapse`); 6h alert throttle; flag `output_collapse_alert` default ON. (b) **KLINE.EMPTY.TELEMETRY.1** — gate keys `KLINE_EMPTY` (all TFs empty = API-outage signature) / `KLINE_PARTIAL` (<60 candles) recorded in `scan_coin()` Step 1 with per-exchange Prometheus labels; `httpx.TimeoutException` branches in both kline fetchers feed `klineTimeouts:spot/futures` hash fields via the existing 5s batched flush. (c) **EARLY.BREAKOUT.PENALTY.1** — `early_breakout_score_adj()` pure fn: −8 setup score for BUY+EARLY_BREAKOUT, applied to `effective_score` before AI/confidence; flag `early_breakout_penalty_v1` default OFF; SELL-side EARLY untouched (it's alpha, WR 68%). (d) **BB.EXPANSION.RETIREMENT.1** — verified already retired in `breakout_intelligence.py`; locked with behavioral + static regression tests instead of a dead re-enable flag. (e) **ATTRIBUTION.SNAPSHOTS.1** — `attribution_snapshots` table (`database/attribution-snapshots-migration.sql`) + `backend/analytics/outcome_learning.py` (17 dimension sets incl. `regime|type|breakout` probability-lookup triple, MIN_CELL_N=10, 90d retention) + nightly Celery task 00:15 UTC; flag `attribution_snapshots` default ON. scan_coin now reads FeatureFlags ONCE per coin and reuses for gate v2 + penalty. 28 new tests in `test_p0_expectancy_recovery.py`. See `docs/PHASE_9_P0_EXPECTANCY_RECOVERY_1.md`.
48. **REGIME.HARD.GATE.V2 (June 2026, flag-gated)** — `FeatureFlags.regime_hard_gate_v2` (default **OFF**). When ON, `contra_regime_gate()` in `signal_pipeline.py` (Step 10.5, after futures intelligence so OI is available) hard-rejects BUY in BEAR_TREND/CAPITULATION and SELL in BULL_TREND/EUPHORIA **unless** `breakout_strength == HIGH_MOMENTUM_BREAKOUT` or aligned OI (`NEW_LONGS` for BUY / `NEW_SHORTS` for SELL). When OFF, legacy behavior: unconditional BUY-in-bear reject (REGIME_REJECTION) + SELL-in-bull soft +10 confidence. ALPHA.TRUTH.1 NULL-regime gate untouched (runs after, owns NULL path). Telemetry: `CONTRA_REGIME_REJECTION` in `GATE_REJECTION_KEYS`/`_PERSISTED_GATE_KEYS` + System gate grid. Dashboard: `RegimeHardGateCard` in Trading→Regime (status toggle via `features` group patch, 24h/7d rejections, est. avoided loss = count × 0.405R). Data basis: contra-regime BUY n=200 WR=19% Exp=−0.405R; HIGH_MOMENTUM override cohort WR=81.8%. 21 unit tests in `test_regime_hard_gate_v2.py`. See `docs/PHASE_9_ALPHA_MAXIMIZATION_1.md` §9.
60. **TELEGRAM.SIGNAL.ONLY.1 (June 2026)** — Telegram sends only signal alerts by default. All ops/anomaly messages gated behind `telegram.ops_alerts_enabled` (default **false**). Five alert paths now check this flag before sending: (1) critical anomaly alerts (`burn_in.py`), (2) scan failure alerts (`scan_task.py` — reads module-level `_scan_ops_alerts_enabled` flag updated async at scan start), (3) Claude degradation alerts (`ai_validator.py`), (4) signal output collapse alerts (`monitoring.py` → `send_output_collapse_alert` now async), (5) Binance geo-block alerts (`market_fetcher.py`). Also: `send_scan_summary` (fired after every scan) + `send_provider_fallback_alert` (CMC cache cold) gated via same flag. `daily_summary_enabled` default changed `true → false`. Enable ops alerts from Admin → Settings → `telegram.ops_alerts_enabled`. `_ops_alerts_enabled()` async helper in `telegram_notifier.py` is the single check point. Additionally: AI disabled by settings toggle no longer calls `_record_call_outcome(is_fallback=True)` — intentional off is not a degradation event; `anomaly_detector.check_ai_health()` + `monitoring._detect_anomalies()` both accept `ai_enabled` param to skip fallback-rate checks when AI is off. Commits `5270348` + `70c7f93` + `ebf472f`.
61. **Worker HEALTHY display bug (June 2026, commit `70c7f93`)** — `app/admin/trading/page.tsx` was checking `healthReady?.checks?.celery_worker === 'ok'` but `/health/ready` returns `'HEALTHY'` / `'DEGRADED'` / `'OFFLINE'` (never `'ok'`). Result: worker always showed as DOWN regardless of actual state. Fixed: `workerOk = checks.celery_worker === 'HEALTHY'`; DEGRADED shown in amber, DOWN in red. System page `ServiceCard` already handled these values correctly — only the trading page was broken.
62. **PLATFORM.SIMPLIFICATION.1 — 5 centers → 3 centers (June 2026, branch `feat/platform-simplification-1`)** — Admin dashboard consolidated from 5 centers (trading, intelligence, analytics, system, settings) + 15 tabs into 3 centers + 9 tabs. **`/admin/signals`** (new): Overview + merged Signals/Tactical (lifecycle preset buttons: Active/Won/Lost/Expired/All; LifecycleFunnel surfaced; AlphaWatchlist collapsed) + Regime — ScannerTab removed. **`/admin/performance`** (new): Track Record (first/default) + Edge + Attribution (Calibration merged as collapsed section; IntelligenceValidationSection deleted; ProbabilityTab deleted as duplicate). **`/admin/system`** (updated): Health + Anomalies + Settings (new tab with Quick Controls, Operating Mode, Feature Flags, Advanced accordion — extracted from old settings/page.tsx). Redirects: `/admin/trading`→`/admin/signals`, `/admin/analytics`→`/admin/performance`, `/admin/intelligence`→`/admin/system?tab=system`, `/admin/settings`→`/admin/system?tab=settings`. Phase D deletions: `app/api/news/grok/route.ts` deleted; NewsTab removed from intelligence page; MarketStructureBreakdown removed from system page; BUY/SELL balance chips + confidence distribution strip removed from signals page. Old page files (`trading/`, `analytics/`, `intelligence/`, `settings/`) remain in repo but are unreachable via next.config.mjs redirects — delete after verifying new centers work.
63. **SIGNAL.QUALITY.AUDIT.3 — Feature flags UI overhaul + P0 recommendations (June 2026, commits `caa3948` + `acb1514`)** — Three UI improvements to `app/admin/system/page.tsx` driven by signal quality audit (7D WR=20%, confidence miscalibration, heuristic grade inversion). (1) **Toggle visibility** — `bg-bull-default/80` (dark, indistinguishable from OFF) replaced with `bg-emerald-500` ON / `bg-zinc-600` OFF; thumb `bg-white` ON / `bg-zinc-300` OFF — states are now immediately readable. (2) **`FLAG_META` + 3-tier `FeatureFlagCard`** — `FLAG_META` constant categorizes all 20 `features` group flags into `quality` (7 flags, signal impact) / `operational` (6 flags) / `advanced` (6 background/analytics flags, collapsed by default). `FeatureFlagCard` gains `ON`/`OFF` chip (emerald/zinc), amber border + `↑ enable`/`↓ disable` action badge when a P0 flag is not at its recommended state, and a reason line (e.g. `0/9 wins last 7D · disable to stop active losses`). (3) **5 P0 flags with "Apply All Recommended" button** — `high_confidence_mode_enabled` (recommended OFF: 0/9 wins 7D), `regime_hard_gate_v2` (ON: contra-regime BUY 19% WR), `early_breakout_penalty_v1` (ON: BUY+EARLY_BREAKOUT unpenalized), `probability_gate_v1` (ON: 2/3 live signals in WR<40% cohorts), `riskgrade_v2` (ON: heuristic grades inverted A<C). A `N P0 fixes pending` amber badge + green `✓ Apply All Recommended` button appear when any P0 flags are not at recommended state; button patches all in one `adminApi.settings.patch('features', changes)` call. See `docs/SIGNAL_QUALITY_AUDIT_3.md`.
64. **SYSTEM.DIAGNOSTICS.1 — Health tab cleanup + ops hardening (June 2026, commits `caa3948` + `acb1514`)** — (1) **Duplicate feature flags removed from Health tab** — `AdvancedOperationsAccordion` in Health tab had its own 5-flag feature flags section (same flags now in Settings tab). Section removed; accordion renamed "System Diagnostics"; subtitle updated to `· provider health · queue metrics · gate analysis · infra config`. `ADVANCED_FLAG_DEFS` constant removed. Feature flags now live in exactly one place: System → Settings → Feature Flags. (2) **Telegram health check false-DOWN fix** — `checkTelegram()` in `app/api/health/providers/route.ts` was calling `api.telegram.org/getMe` from Vercel, causing 4001ms timeout false-DOWN readings (Vercel geo-restricts Telegram). Fixed: token-presence check only — configured = healthy; delivery health covered by CloudAMQP/worker heartbeat check. (3) **AI degradation alert fail-safe** — exception handler in `_send_degradation_alert()` (`ai_validator.py`) was `pass` (fall-through to re-raise); changed to `return` so a failed alert send doesn't crash the validation path. AI-disabled path no longer calls `_record(used_fallback=True)` — intentional OFF is not a degradation event. (4) **Coordinator TTL** — `last_scan_ts` Redis key in `coordinator.py` now set with `ex=7*24*60*60` (7-day TTL) to prevent orphaned key accumulation on redeploys.
65. **UI.UX.MODERNIZATION.1 — Visual polish pass across all 3 admin centers (June 2026, commit `498ca4a`, UI-only)** — 25-item discipline pass: zero logic/API/DB changes. **P0 (safety/clarity):** Emergency Stop toggle red when ON (`Toggle danger` prop); Scan "Queued ✓" button blue not green; sidebar sub-text normal-case no tracking; SystemStatusBanner neutral zinc when OK (green reserved for real positive events); global `focus-visible` ring in `globals.css`. **P1 (sprint):** Full type scale normalization — no `text-[9px]`/`text-[11px]`, `tracking-widest` → `tracking-wide` on section labels; lifecycle badges 10 colors → 4 semantic groups (active=blue, won=emerald, lost=red, closed=zinc); BUY/SELL `text-sm font-bold`, ConfBar + FreshnessTag + RegimeAlignDot removed from collapsed signal row; compact service grid when all healthy; TrackRecord 3-col hero grid (WR/Exp/PF at `text-2xl`); mode chips uniform zinc; `glass-card` solid `bg-zinc-900` (removed `backdrop-filter: blur` — invisible on dark bg, was adding GPU compositor cost); tab labels `text-xs font-medium` (removed mono + uppercase); preset filter pills (`rounded-full`); ops card destructive action divider. **P2 (polish):** `IntelligencePanel` rewritten as 4 collapsible sections (Signal Quality always-open / Intelligence open / Technical closed / AI Analysis open) with `▸ show / ▾ hide` affordances; `ConfidenceBar` component above signal list (proportional 4-segment bar 90+/85-89/80-84/<80); `DimTable` top-3 default with "Show all N rows" expand; `WrSparkBar` 40px inline bar in DimTable; Anomalies calm zero state (single neutral line when no anomalies); `FeatureFlagCard` description hidden/hover-reveal; card headers unified to `text-sm font-medium text-white`. See `docs/UI_UX_MODERNIZATION_IMPLEMENTATION_1.md`.
