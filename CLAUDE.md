...
app/admin/
  trading/page.tsx      ← Trading Operations — 5 tabs: overview(scanner+regime+metrics+recent signals) · scanner(celery+ops toggles+gate analysis) · signals(feed, /api/signals/tactical) · tactical(lifecycle preset filter) · regime(apply regime settings)
  intelligence/page.tsx ← Intelligence Center — 4 tabs: providers(8-service health) · cache(CMC quota+groups+workers) · sectors(category cards) · market(regime+trending+news)
