...
51. **SETTINGS.SIMPLIFY.1 (June 2026)** — Founder Control Center reduced from 5 sections + 3 accordions to 3 sections + 1 accordion. Page order: FounderSummaryCard → **Quick Controls** (audited daily set, always visible: `ai.enabled` + `telegram.alerts_enabled` toggles via FeatureFlagCard, then `scanner.min_confidence`/`scanner.alert_confidence`/`signals.min_rr_ratio`/`scanner.max_coins_per_run` via TacticalControlCard) → Operating Mode (3 primary cards + 4 specialist preset **chips**, no accordion; sub-85-confidence presets carry an ALPHA.TRUTH.1 warning) → "All Settings & Audit Log" single collapsed accordion (two-column group browser unchanged inside). Removed: ActiveSettingsSummary (duplicated FounderSummaryCard), Advanced Presets + Advanced Settings accordions. `anomaly` group added to HIDDEN_GROUPS — `anomaly_detector.py` uses its own constants; the DB overrides were never read (dead UI). Ops toggles (emergency stop/maintenance/scheduler) intentionally live ONLY in Trading→Scanner; settings page deep-links there via `/admin/trading?tab=scanner`.
