...
class ScannerSettings(BaseSettingsGroup):
    ...
    min_empirical_exp: float = Field(
        0.0, ge=-2.0, le=5.0,
        title='Min Empirical Expectancy (R)',
        description='PHASE.9.P1 — additional delivery floor on cohort expectancy (mean realized R). Only enforced when BOTH Probability Gate and the Expectancy Filter flags are ON. Unknown expectancy never gates.',
    )
...
class FeatureFlags(BaseSettingsGroup):
    ...
    telegram:                bool = Field(True,  title='WhatsApp Alerts',          description='Master switch — all WhatsApp signal alerts disabled when off (field key is "telegram" for DB compatibility)')
    ...
    regime_hard_gate_v2:     bool = Field(True,  title='Regime Hard Gate V2',      description='REGIME.HARD.GATE.V2 — symmetric contra-regime hard gate (BUY in bear, SELL in bull) with HIGH_MOMENTUM breakout / aligned-OI override paths. OFF = legacy behavior (unconditional BUY-in-bear gate only). Recommended ON — contra-regime BUY cohort WR=19%, HIGH_MOMENTUM override WR=81.8%.')
    early_breakout_penalty_v1: bool = Field(True,  title='Early Breakout BUY Penalty', description='EARLY.BREAKOUT.PENALTY.1 — apply −8 setup-score penalty to BUY signals with EARLY_BREAKOUT strength before confidence calculation. Recommended ON — BUY-side EARLY cohort WR ≈17–27%; SELL-side EARLY is alpha (WR 68%) and is untouched.')
    ...
    high_confidence_mode_enabled: bool = Field(False, title='High-Confidence Mode', description='Run the high_confidence scan cycle (every 30 min). Recommended OFF — audit: 26.8% WR 30d global, 0/9 wins last measured week. Futures mode covers the same universe with better selection.')
    probability_gate_enabled: bool = Field(True,  title='Probability Gate (WhatsApp)', description='PHASE.9.1 — suppress WhatsApp delivery for signals whose empirical win probability (from attribution snapshots, n≥30 cohort) is below scanner.min_empirical_wr. Signals are still generated, persisted, and outcome-tracked — only the alert is withheld. OFF = deliver on confidence threshold alone (legacy).')
    probability_gate_v1:      bool = Field(True,  title='Probability Gate — Expectancy Filter', description='PHASE.9.P1 — additionally require cohort expectancy ≥ scanner.min_empirical_exp for Telegram delivery (AND-combined with the win-rate gate). Needs Probability Gate ON to have any effect. Unknown expectancy never gates.')
    riskgrade_v2:             bool = Field(True,  title='RiskGrade 2.0 (empirical grades)', description='PHASE.9.P1 — display outcome-derived grades (A+/A/B+/B/C/D from cohort expectancy) as the primary grade in dashboards. Empirical grades are perfectly monotonic (A+ 73.5%/+1.286R → D 13.6%/−0.581R); heuristic grades are inverted (A < C). Heuristic A–F remain stored.')
